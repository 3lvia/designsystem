...
export class VersionService {
  ...
  updateVersionFromCDN(): void {
    this.http.get<any[]>('https://cdn.elvia.io/npm/filelist.json').subscribe(data => {
      ...
      const scriptFile = elvisSorted.find(item => item.filename.indexOf('elvis-' + this.cdnVersion + '/elvis.js') > -1);
      const styleFile = elvisSorted.find(item => item.filename.indexOf('elvis-' + this.cdnVersion + '/css/elvis.min.css') > -1);
