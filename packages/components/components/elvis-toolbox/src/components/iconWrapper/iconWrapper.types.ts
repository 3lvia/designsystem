...
export interface IconWrapperProps<TIcon extends { getIcon: (color?: ColorLabel | AnyString) => string }>
  extends React.DetailedHTMLProps<React.HTMLAttributes<HTMLElement>, HTMLElement> {
  /**
   * The icon to render from `@elvia/elvis-assets-icons`.
   */
  icon: TIcon;
  color?: ColorLabel | AnyString;
  /**
...
export type IconWrapperType<
  TIcon extends { getIcon: (color?: ColorLabel | AnyString) => string } = {
    getIcon: (color?: ColorLabel | AnyString) => string;
  },
