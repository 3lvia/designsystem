...
async function createEmbeddedIconsJS() {
  const iconsToInclude = icons.map((i) => {
    if (i.deprecated) {
      return `src/icons/svg/src/${i.newIconName}.svg`;
    } else {
  ...
  for (let i = 0; i < deprecatedIcons.length; i++) {
    embeddedJs =
      ...
      `{
        ...
        version: "${deprecatedIcons[i].version.toString()}",
...
async function createIconModule() {
  const iconsToInclude = icons.map((i) => {
    ...
    if (i.deprecated) {
      return {
        name: i.name,
        path: `src/icons/svg/dist/${i.newIconName}.svg`,
      };
    } else {
      return {
        name: i.name,
