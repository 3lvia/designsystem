(() => {
    ...
    function replaceIcons() {
        window.document.querySelectorAll('[class*="e-icon"]').forEach(element => {
    ...
    function getIcon(classList) {
        for(let i = 0; i < classList.length; i++) {
            ...
            if(icons[classList[i].value]) {
                return '<img width="100%" height="100%" aria-hidden="true" src="' + icons[classList[i].value]  + '"></img>';
            }
        }
