...
export class IframeScreenComponent implements AfterViewInit {
  ...
  createIframe() {
    if (this.screenSize === 'desktop') {
      ...
      doc.write(`<html><head>${window.document.head.innerHTML}</head><body style="padding:16px 8px; overflow: hidden;">${this.code}</body></html>`);
      doc.close();
    ...
    if (this.screenSize === 'tablet') {
      ...
      doc.open();
      // tslint:disable-next-line:max-line-length
      doc.write(`<html><head>${window.document.head.innerHTML}</head><body style="padding:4px; overflow: hidden;">${this.code}</body></html>`);
      doc.close();
