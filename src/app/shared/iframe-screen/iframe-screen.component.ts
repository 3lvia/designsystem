...
export class IframeScreenComponent implements AfterViewInit {
  ...
  createIframe() {
    ...
    if (this.screenSize === 'desktop') {
      ...
      doc.write(`<html><head>${window.document.head.innerHTML}</head><body><div style="max-height: 500px; overflow: auto; padding: 8px;">${this.code}</div></body></html>`);
      doc.close();
    ...
    if (this.screenSize === 'tablet') {
      ...
      doc.open();
      // tslint:disable-next-line:max-line-length
      doc.write(`<html><head>${window.document.head.innerHTML}</head><body><div style="max-height: 470px; overflow: auto; padding: 4px;">${this.code}</div></body></html>`);
      doc.close();
